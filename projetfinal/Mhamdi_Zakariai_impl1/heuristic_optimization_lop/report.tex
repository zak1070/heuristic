...
The exponential growth of the solution space in combinatorial optimization makes exact methods impractical for large instances. For this reason, heuristic algorithms are often used, as they trade strict optimality guarantees for practical efficiency.

This report evaluates iterative improvement algorithms and Variable Neighborhood Descent (VND) applied to the Linear Ordering Problem (LOP), an NP-hard combinatorial problem. The objective is to implement and compare several algorithms by combining different initialization methods, neighborhood structures, and pivoting rules.
...
This is equivalent to simultaneously reordering the rows and columns of $C$ to maximize the sum of all entries strictly above the main diagonal. Since the number of permutations is $n!$, exhaustive search is computationally infeasible even for moderate $n$, fully justifying the use of local search heuristics. The instances used in this study have sizes $n = 150$ and $n = 250$.
...
When a neighborhood is defined, a \emph{pivoting rule} determines which neighbor to select:
...
\label{tab:tests}
% On remplace le deuxième 'l' par 'p{5cm}' pour forcer le retour à la ligne
\begin{tabular}{l p{5cm} ccc}
\toprule
...
The CW heuristic consistently and substantially outperforms random initialization, reducing the average deviation by 6.55 percentage points across all paired variants. Both the $t$-test ($t = -16.00$, $df = 467$) and the Wilcoxon test ($p < 2.2 \times 10^{-16}$) confirm this advantage is highly statistically significant. CW, as a constructive greedy method that maximizes local attractiveness, places the starting solution in a region of the search space much closer to good local optima than a uniform random permutation, accelerating subsequent local search convergence.

However, the performance gain from CW initialization is heavily topology-dependent. The 6.55 percentage point average reduction is severely skewed by the weak Transpose neighborhood (where CW reduces deviation by $\sim 18.7$ points). When utilizing the powerful Insert topology, the gap between CW and Random initializations is drastically reduced to 0.33 percentage points (1.691\% vs. 2.021\%). A sufficiently strong local search operator thus largely compensates for an uninformed starting state, though the structural advantage of CW remains small but consistent: the exploratory depth of Insert mitigates, but does not entirely eliminate, the penalty of a poor initial permutation.

\subsection{Impact of Neighborhood Structure}

Neighborhood structure is the primary determinant of solution quality. The Insert operator yields the lowest average deviations (1.69--2.30\%), followed by Exchange (2.41--3.60\%). Conversely, the Transpose neighborhood performs poorly, resulting in average deviations between 15.73\% and 34.47\%.

These performance differences align with the structural properties of each neighborhood. Transpose, limited to adjacent swaps with a size of $\mathcal{O}(n)$, makes only micro-adjustments and quickly converges to poor local optima (typically in under 10~ms). While Exchange explores a larger $\mathcal{O}(n^2)$ neighborhood, swapping distant elements disrupts the relative order of intermediate components, which can degrade well-optimized partial sequences. Insert, however, relocates a single element while preserving the relative order of the remaining sequence. This localized disruption is structurally well-suited to the LOP, as it directly supports the objective of optimizing a global ordering criterion.

\subsection{Impact of Pivoting Rule}

Analysis of the pivoting rules within the Insert neighborhood (using CW initialization) demonstrates that First-Improvement achieves significantly better solution quality than Best-Improvement. Specifically, CW+Insert+First reaches an average deviation of 1.691\% compared to 1.841\% for the Best-Improvement variant. This difference is statistically significant, as confirmed by both the paired $t$-test ($p = 8.74 \times 10^{-4}$) and the Wilcoxon signed-rank test ($p = 6.82 \times 10^{-4}$).

These results can be attributed to the differing search dynamics of each rule. Best-Improvement follows the steepest local gradient by evaluating the entire neighborhood at each iteration, leading to rapid convergence (about  27s) to the nearest local optimum. In contrast, First-Improvement accepts the first improving move found and immediately restarts the neighborhood traversal. This behavior results in a longer (about  113s) and more diverse trajectory through the search space, allowing the algorithm to explore a wider variety of improving directions and settle into deeper basins of attraction. While the execution time is approximately four times higher, the increased computational effort is justified by the gain in final solution quality.
...
This result matches what we would expect given how the algorithm works. The VND mechanism ensures that, regardless of ordering, the algorithm eventually exercises all three neighborhoods before terminating. The Insert neighborhood, which provides the most powerful improvements, is reached in both variants. The intermediate neighborhoods (Transpose and Exchange) serve primarily to escape from local optima that the dominant neighborhood cannot resolve, and their relative order has little bearing on the final result.
...
VND\_TIE is therefore the recommended configuration: a 31\% reduction in runtime with no measurable loss in solution quality.

\section{Conclusion}
\label{sec:conclusion}

In this work, we tested twelve iterative improvement algorithms based on different combinations of neighborhoods, pivot rules, and initializations.

Among all design choices, \textbf{neighborhood structure} is by far the most decisive. Insert far outperforms Exchange and Transpose: by relocating a single element while preserving the relative order of all others, it is structurally well-suited to an objective that is inherently an ordering criterion. Transpose, limited to adjacent swaps, gets trapped in very shallow local optima and is essentially not competitive.

The \textbf{pivoting rule} also has a statistically significant effect, and in a counterintuitive direction: First-Improvement yields better solutions than Best-Improvement. Rather than converging greedily to the nearest local optimum, it follows a longer and more erratic trajectory that explores a richer set of improving directions. The cost is a fourfold increase in runtime (113s vs.\ 27s for CW+Insert).

\textbf{Initialization} matters, but less than the above. The CW heuristic reduces average deviation by 6.5 percentage points over random generation overall --- a gap driven almost entirely by the Transpose neighborhood. Under Insert, the difference shrinks to 0.33 points.

Finally, \textbf{VND} provides no quality gain over CW+Insert+First in this setting: the Insert operator alone is sufficient to reach high-quality local optima. Between the two orderings, VND\_TIE (Transpose $\to$ Insert $\to$ Exchange) is preferred for its 31\% lower runtime with statistically equivalent solution quality.
