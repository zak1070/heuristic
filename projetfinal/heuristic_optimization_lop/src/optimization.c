...
/*
 * Computes the absolute objective function value from scratch.
 * Formula: f(s) = sum_{row < col} CostMat[solution[row]][solution[col]]
 * Complexity: O(n^2). Only used for initial evaluation.
 */
...
/* * Generates a uniformly random permutation (Uninformed Random Picking).
 */
...
/* Helper struct for the CW heuristic: pairs an element ID with its attractiveness score */
typedef struct {
    long int      element_id;
    long long int attractiveness;
} CWItem;

/* Descending comparator on attractiveness score for qsort */
...
void vnd_tei(long int *solution, long long int *current_cost) {
    ...
    while (neighborhood_index < 3) {
        ...
        if (*current_cost > cost_before)
            neighborhood_index = 0; /* improvement found: restart from Transpose */
        else
            neighborhood_index++;   /* no improvement: try next neighborhood */
    }
}
