...
for file in instances/N-*; do
    ./lop_ls --instance "$file" >> raw_data.txt
done
...
# Check if the best_known file exists
...
while IFS=',' read -r instance init algo cost time_s; do
    ...
    # Clean the instance name (e.g., 'instances/N-xxx' becomes 'N-xxx')
    ...
    if [ -n "$best_cost" ] && [ "$best_cost" -ne 0 ]; then
        ...
        # Write the formatted data into the final txt file
